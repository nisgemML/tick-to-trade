...
int main() {
    ...
    CHECK(r.events_submitted == 2000, "submitted count");
    CHECK(r.events_accepted == 2000, "all accepted under light load");
    CHECK(r.fills > 0, "synthetic stream must produce fills");
    CHECK(r.queue_full_rejects == 0, "no rejects under light load");
